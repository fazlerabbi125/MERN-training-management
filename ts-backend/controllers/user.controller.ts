...
class UserController {
    ...
    async signup(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const accessToken = jwt.sign(userData, JWT_keys.access, {
                expiresIn: process.env.JWT_EXPIRATION,
            });
            const refreshToken = jwt.sign(userData, JWT_keys.refresh, { expiresIn: "7d" });
        ...
        try {
            ...
            if (user) {
                ...
                return res
                    ...
                    .send(failure("Refresh token successfully deleted!"));
        } catch (error) {
            console.log(error);
            next(error);
        }
